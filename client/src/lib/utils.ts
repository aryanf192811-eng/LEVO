...
export const vehicleStatusConfig: Record<string, { label: string; className: string }> = {
...
export const driverStatusConfig: Record<string, { label: string; className: string }> = {
  AVAILABLE:  { label: 'Available',  className: 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200' },
  ON_TRIP:    { label: 'On Trip',    className: 'bg-blue-50 text-blue-700 ring-1 ring-blue-200' },
...
export const tripStatusConfig: Record<string, { label: string; className: string }> = {
...
export function statusToBadge(status: string): string {
  const allConfigs = { ...vehicleStatusConfig, ...driverStatusConfig, ...tripStatusConfig }
  return allConfigs[status]?.className || 'bg-slate-100 text-slate-600 ring-1 ring-slate-200'
}
...
export function fmtDateRelative(dateStr: string | Date): string {
...
export function licenseExpiryClass(dateStr: string | Date): string {
